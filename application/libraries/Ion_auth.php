		...
		if ($user)
		{
			...
			if ($code)
			{
				...
				else
				{
					...
					if(CI_send_email($user->email, get_setting_general('name').' - '.$this->lang->line('email_forgotten_password_subject'), $message))
					// if ($this->email->send())
					{
						$this->set_message('forgot_password_successful');
						return TRUE;
	...
	public function register($identity, $password, $email, $additional_data = [], $group_ids = [])
	{
		...
		else
		{
			...
			else
			{
		        ...
		        if($this->my_phpmailer->send($email, get_setting_general('name') . ' - ' . $this->lang->line('email_activation_subject'),$message)){
		            // echo '1';
		            return $id;
		        }
