...
class Ajax_pagination_admin{
    ...
    var $next_link       = '<span class="glyphicon glyphicon-triangle-right"></span>';
    var $prev_link       = '<span class="glyphicon glyphicon-triangle-left"></span>';
    var $last_link       = '<span class="glyphicon glyphicon-step-forward"></span>';
    var $uri_segment     = 4;
    var $full_tag_open   = '<ul class="pagination index-pagination">';
    ...
    function create_links(){
        ...
        if ($this->show_count){
            ...
            $info = '  <li style="line-height:32px; padding-right: 10px; float: left;">顯示 ' . ( intval($curr_offset) + 1 ) . ' - ' ;

            if( ( intval($curr_offset) + $this->per_page ) < ( $this->total_rows -1 ) )
            $info .= intval($curr_offset) + $this->per_page;
            else
            $info .= $this->total_rows;

            $info .= ' 筆 / 共 ' . $this->total_rows . ' 筆</li>';
            $output .= $info;
        }
